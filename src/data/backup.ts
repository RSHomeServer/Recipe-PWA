...
function parseAllTables(data: BackupV1["data"]): ParsedBackupData {
  ...
  return {
    ingredients: parseArray("ingredients", IngredientSchema, data.ingredients),
    ...
    loggedMeals: parseArray("loggedMeals", LoggedMealSchema, data.loggedMeals),
...
export async function exportBackup(db: Dexie): Promise<BackupV1> {
  const [
    ...
    RECIPE_TABLE_NAMES.map((name) => db.table(name).toArray()),
  );

  const recipeImages = await Promise.all(
    (recipeImagesRaw as RecipeImage[]).map(async (row) => {
 ...
 * Never merges — existing rows are cleared first.
 */
