...
| ID | Requirement |
| --- | --- |
| R2.1 | `Ingredient` MUST gain a non-nullable `source: IngredientSource` (shape in ADR-002 §1), a nullable `imageId`, and a `common: boolean`. |
| R2.2 | `source` and `common` MUST NOT be read by any calculation or derivation. Guarded by property test. `common` governs picker ordering only, and defaults to `true` for user-created ingredients. |
...
| R2.5 | The starter pack MUST be **generated by a build-time transcode script** from the published dataset. **Nothing is transcribed by hand.** Prefer UK CoFID; USDA FoodData Central is a sanctioned fallback needing no escalation if CoFID's format makes the script materially slower. |
| R2.5a | The pack MUST include **every generic entry with complete macro data** — filter out entries missing any of kcal, protein, carbohydrate or fat rather than guessing. Expect several hundred, not ~150. This is the "wide base" the photo trade-off bought. |
| R2.5c | The script MUST read the **`Proximates` worksheet** of the CoFID Excel workbook, taking `KCALS` (not `KJ`), `PROT`, `FAT` and `CHO`, with the food code as `source.entryCode` and the **`GROUP` column** driving category mapping. **Column headings occupy rows 1–3**; a single-header-row reader will misparse. The separate 'old foods' file MUST be excluded. Verified details in [ADR-002](../adr/002-reference-ingredient-data-and-provenance.md) §2a. |
| R2.5d | **`Tr` MUST map to 0; `N` MUST exclude the entry.** They are not the same: `Tr` is a trace, `N` means the nutrient is present in significant quantities with no reliable figure. Mapping `N` to 0 asserts the opposite of what the dataset says, systematically and silently. This is the highest-consequence line in ticket 5. |
| R2.5e | The transcode MUST assign `measureKind: "mass"` for per-100g entries and `"volume"` for alcoholic beverages (per 100ml). It MUST NOT attempt to seed `count` ingredients — CoFID contains no per-item data, and decision 2 already excludes per-item weights. Count ingredients stay user-created. |
| R2.5b | A hand-maintained list of **~150 source entry codes** MUST mark `common: true`. This list is the only hand-authored part of the pack. A code matching no seeded entry MUST fail the build. |
| R2.6 | The common list MUST cover the stated weekly pattern on day one: chicken thigh and wing (raw, skin on and off), frozen hashbrowns, frozen peas, mixed vegetables, broccoli, and common table sauces including BBQ. |
...
| R2.9a | Ingredient pickers MUST show `common` entries first and place the rest behind an explicit "show all N results". Without this the wide base is unusable: a raw composition table returns a dozen states of chicken to someone trying to plan dinner. |
| R2.9b | Brand-specific products are **the user's job in V2**, entered by hand as `source.kind: "packaging"`. No automated brand onboarding, no online lookup. |
...
| R2.12 | Licence attribution MUST appear in Settings → About. **A human MUST resolve the licence version and exact attribution string from the GOV.UK publication page's own licence footer before release.** Secondary sources disagree between Open Government Licence v1 and v3.0, and the user guide carries only a Crown copyright notice — see [ADR-002](../adr/002-reference-ingredient-data-and-provenance.md) §2b. Neither the ADR nor a search summary is sufficient. |
...
| R5.9 | **There MUST be no setting.** No `controlStyle`, no compact mode, no route back to native selects. The rule applies everywhere. Consequently every segmented group MUST be verified at a 320px viewport — wrapping, not truncating, not below 44px — as a release gate, because nothing catches it if it slips. |
...
| --- | --- | --- |
| `Ingredient` | `+ source: IngredientSource` (non-null), `+ imageId: Id \| null`, `+ common: boolean` | `source = { kind: "userEntered", all origin fields null, note: null }`; `imageId = null`; `common = true` |
...
| `Settings` | `+ howItWorksDismissed`, `+ starterPackVersion` | Defaults via the existing `normalizeSettingsRow` pattern |
...
| 2 | **Choice control primitives** | R5.1–R5.10. `SegmentedGroup` and `CommandPicker` in `src/ui/`, migrate existing call sites. No setting — see R5.9. | — | 005 |
...
| 5 | **Starter ingredient pack** | R2.5–R2.9b, R2.12. The **transcode script** (Excel input — needs an xlsx reader as a **devDependency only**, never shipped to the browser), the hand-maintained common-code list, build-time validation, first-run seed, Settings top-up, `common`-first picker ordering, attribution. Read ADR-002 §2a before starting; four of its findings change the implementation. | 4 | 002 |
...
2. **Provenance invariance (R2.2)** — permute `source.kind` across all four values and
   `common` across both over a corpus, and assert every derived result is unchanged.
3. **Starter pack integrity** — build-time schema validation, plus spot-checks asserting named
   entries against their published figures and `entryCode`s. **Cover at least one entry per
   category**, plus one `volume` entry alongside the `mass` ones: the script's failure mode is
   a systematic mapping error and a sample of one shape cannot see it. Do not require a
   `count` entry — the dataset has none (R2.5e).
3b. **Sentinel handling (R2.5d)** — against named real entries, assert that a `Tr` macro seeds
   as 0 and that a food carrying `N` in any of the four macros is not seeded at all. Synthetic
   fixtures alone do not prove the real workbook parses as expected.
3a. **Common list resolves** — every hand-maintained code marks a seeded ingredient. A code
   matching nothing fails the build rather than quietly shrinking the default set.
...
9. **Migration** — a V1 database opens at schema v2 with every ingredient at
   `kind: "userEntered"` and `common: true`, every `group` null, and **no nutrition figure
   changed**.
...
11a. **Segmented groups at 320px** — every group in the product wraps without truncating a
    label, overflowing, or dropping below 44px. A release gate, because R5.9 leaves no
    fallback behind it.
...
the shape ready; not built) · **automated brand-specific onboarding** — a considered follow-up,
but in V2 the user adds branded products by hand as `source.kind: "packaging"` · photos of
generic foods from a third-party source (ADR-002 §3) · **any setting controlling control
style** — ADR-005's rule applies everywhere, with no compact mode · cross-family unit
conversion · expiry, lots and waste tracking · multi-user, sync, accounts · macro targets ·
