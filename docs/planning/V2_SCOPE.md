...
| 5 | **Starter ingredient pack** | R2.5–R2.9b, R2.12. The **transcode script**, the hand-maintained common-code list, build-time validation, first-run seed, Settings top-up, `common`-first picker ordering, attribution. | 4 | 002 |
...
3. **Starter pack integrity** — build-time schema validation, plus spot-checks asserting named
   entries against their published figures and `entryCode`s. **Spot checks must cover at
   least one entry per `measureKind` and one per category**: the script's failure mode is a
   systematic mapping error, and a sample of one shape cannot see it.
